...
Small LFR/MINT demos for one-off runs. Neptune `run_all_Quick_Examples.sh` synthesizes **top-level** `*.lfr` / `*.mint` only (**14** LFR + **2** native MINT → `*_PR.json`). One-off `fluigi` without `-o` writes `<Neptune>/Output/<stem>/` instead of `Results/`.
...
| `mixer_3to1.lfr`, `diy_with_mixer_demo.lfr`, `test_DIY_fork.lfr`, `test_DIY_crossing.lfr` | Mixer / DIY demos. DIY keepout: `componentSpacing`, default **2000** µm (`library/DIYcomponent.lfr`) |
| `DIYcomponent.lfr`, `two_in_mixer.lfr`, `three_in_mixer.lfr`, `incubator.lfr`, `droplet_generator.lfr` | Standalone copies of `library/` modules (also synthesized by the batch). Library MUX files keep snake_case stems (`library/mux4to1.lfr`, `library/mux1to4.lfr`). |
| `Quick_Test/test_MUX4.lfr` | `#MAP "MUX" "assign"` 1-to-4 demo (not in the Quick_Examples batch) |
